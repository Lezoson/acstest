...
export const getTokenResponse = async (): Promise<any> => {
  const urlParams = new URLSearchParams(window.location.search);
  const invitationCode = urlParams.get('invitationCode');

  const response = await fetch(`/sendapi?invitationCode=${invitationCode}`)
    .then((res) => {
      return res.json();
    })
    .catch((err) => {
...
export const baseurl =():void=>{
  window.location.href = "https://dev.gtsvirtualhealth.net/";
} 

export const getDisplayNameFromLocalStorage = () => localStorage.getItem('DisplayName');

export const saveDisplayNameToLocalStorage = (props) => localStorage.setItem('DisplayName', props);
